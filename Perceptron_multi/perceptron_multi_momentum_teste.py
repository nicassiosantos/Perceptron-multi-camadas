...
# Função para inicializar as matrizes de pesos e os pesos anteriores
def inicializar_pesos_e_momentum(x, y):
    pesos_atual = np.random.rand(x, y)
    pesos_anterior =  copy.deepcopy(pesos_atual)
    return pesos_atual, pesos_anterior  
...
# Função para atualizar os pesos da camada de saída com momentum
def atualizar_pesos_saida_com_momentum(matriz_camada_saida, matriz_pesos_anterior_saida, Y_camada_oculta, vetor_gradiente_saida):
    for x in range(len(matriz_camada_saida)):
        for y in range(len(Y_camada_oculta)):
            delta_peso = TAXA_APRENDIZADO * vetor_gradiente_saida[x] * Y_camada_oculta[y]
            temporario = matriz_camada_saida[x][y] 
            matriz_camada_saida[x][y] += delta_peso + FATOR_MOMENTUM * (matriz_camada_saida[x][y] - matriz_pesos_anterior_saida[x][y])
            matriz_pesos_anterior_saida[x][y] = temporario  # Armazena o peso anterior

# Função para atualizar os pesos da camada oculta com momentum
def atualizar_pesos_oculta_com_momentum(matriz_camada_oculta, matriz_pesos_anterior_oculta, entradas, vetor_gradiente_oculta):
    for x in range(len(vetor_gradiente_oculta)):
        for z in range(len(entradas) -1): # Ignora o bias nas entradas
            delta_peso = TAXA_APRENDIZADO * vetor_gradiente_oculta[x] * entradas[z]
            temporario = matriz_camada_oculta[x][z]
            matriz_camada_oculta[x][z] += delta_peso + FATOR_MOMENTUM * (matriz_camada_oculta[x][z] - matriz_pesos_anterior_oculta[x][z])
            matriz_pesos_anterior_oculta[x][z] = temporario  # Armazena o peso anterior
...
def teste(data_val, matriz_camada_oculta, matriz_camada_saida):
    acertos = 0
...
def treinamento(datasets):
    ...
    for topologia in topologias:
        ...
        for i in range(10):  # 10-fold cross-validation
            ...
            data_teste = datasets["test"][i]
            eqm_ant = 5
            eqm_atual = 0
            epoca = 0

            # Inicializar matrizes de pesos e momentum
            matriz_camada_oculta, matriz_pesos_anterior_oculta = inicializar_pesos_e_momentum(topologia['oculta'], 5)
            matriz_camada_saida, matriz_pesos_anterior_saida = inicializar_pesos_e_momentum(topologia['saida'], topologia['oculta'] + 1)
            ...
            while abs(eqm_atual - eqm_ant) > ERRO:
                ...
                for j in range(len(data_treino)):
                    ...
                    atualizar_pesos_saida_com_momentum(matriz_camada_saida, matriz_pesos_anterior_saida, Y_camada_oculta, vetor_gradiente_saida)

                    vetor_gradiente_oculta = calcular_gradientes_oculta(vetor_gradiente_saida, matriz_camada_saida, I_camada_oculta)
                    atualizar_pesos_oculta_com_momentum(matriz_camada_oculta, matriz_pesos_anterior_oculta, entradas, vetor_gradiente_oculta)
            ...
            acuracia = teste(data_teste, matriz_camada_oculta, matriz_camada_saida)
...
def main(): 
    base_path = 'Perceptron_multi/iris_teste'
